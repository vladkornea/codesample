...
class DB implements DB_Interface {
	 ...
	 * The value returned by CONNECTION_ID() is the same type of value as displayed in
	 *     the ID column of the INFORMATION_SCHEMA.PROCESSLIST table,
	...
	public static function getConnectionId () : int {
		return static::getCell( 'select connection_id()' );
	} // getConnectionId


	/**
	 * DB::getLastQuery() returns the last query executed by the DB class.
	 */
	...
	/**
	 * Call DB::log() to tell the DB class to keep a log of queries executed.
	 * Call DB::log() again to return the current log; this will also reset the log.
	 * Pass false as the optional argument to disable further logging.
	 * <code>
	 * DB::log();
	 * //etc
	 * $log = DB::log( false );
