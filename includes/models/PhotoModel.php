...
class PhotoModel extends LoggedModel implements PhotoModelInterface {
	...
	static public function create (array $form_data, string $event_synopsis = '', bool $log_query = true) {
		...
		switch ($original_file_extension) {
			case 'jpeg':
			case 'jpg':
				$original_photo_resource = imagecreatefromjpeg($temp_original_photo_path);
				break;
			case 'png':
				$original_photo_resource = imagecreatefrompng($temp_original_photo_path);
				break;
			case 'gif':
				$original_photo_resource = imagecreatefromgif($temp_original_photo_path);
				break;
//			case 'bmp':
//				$original_photo_resource = imagecreatefrombmp($temp_original_photo_path); // PHP 7.2 required
//				break;
			default:
				return ['file' => "Unrecognized image format."];
				break;
		} // switch
		if (!$original_photo_resource) {
		...
		$form_data['original_width'] = imagesx($original_photo_resource);
		$form_data['original_height'] = imagesy($original_photo_resource);
		...
		$success = imagecopyresampled($standard_photo_resource, $original_photo_resource, 0, 0, 0, 0, $form_data['standard_width'], $form_data['standard_height'], $form_data['original_width'], $form_data['original_height']);
		if (!$success) {
