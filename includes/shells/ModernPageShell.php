...
interface ModernPageShellInterface extends HtmlPageShellInterface {
} // ModernPageShellInterface

class ModernPageShell extends HtmlPageShell implements ModernPageShellInterface {
	function __construct (string $page_title = "TypeTango") {
		ob_start();
		parent::__construct($page_title);
		$this->addCssFile('/css/generic.css');
		$this->addCssFile('/css/modern.css');
		$this->addJsFile('/js/lib/jquery/jquery-3.4.1.min.js');
		$this->addJsFile('/js/functions.js');
		JavascriptErrorHandler::register($this);
		$this->addCssFile('/widgets/modern-login-widget/login-widget.css');
		$this->addJsFile('/widgets/modern-login-widget/login-widget.js');
		if (SERVER_ROLE != 'live') {
